  ...
  return (
    <div className={`mx-auto ${isDesktop ? "space-y-6" : "space-y-4"}`}>
      ...
      <div className={`flex flex-col gap-4 ${isDesktop ? "sm:flex-row sm:items-center sm:justify-between p-6" : "p-4"} bg-[#1c1730] rounded-2xl border border-white/10 shadow-2xl relative overflow-hidden`}>
        ...
        <h1 className={`${isMobile ? "text-sm" : "text-lg"} font-black text-white uppercase tracking-[0.2em] flex items-center gap-3 pl-2`}>
          MASTER DATA UNIT
        </h1>
      ...
      <div className={`grid gap-4 ${
        ...
        <StatCard label="Inventory" value={stats.total} icon={<Building2 />} color="bg-blue-500" isCompact={isDesktop || isMobile} />
        <StatCard label="Available" value={stats.tersedia} icon={<CheckCircle2 />} color="bg-emerald-500" isCompact={isDesktop || isMobile} />
        <StatCard label="Sold Out" value={stats.terjual} icon={<X />} color="bg-rose-500" isCompact={isDesktop || isMobile} />
        <StatCard label="Hold" value={stats.booking} icon={<Clock />} color="bg-amber-500" isCompact={isDesktop || isMobile} />
      </div>
          ...
          <table className="table-modern w-full border-collapse">
            <thead>
              <tr className={isDesktop ? "h-14" : "h-12"}>
                <th className="w-12 text-center text-[9px] bg-[#17152b] text-white/30 uppercase tracking-[0.2em] font-black">#</th>
                <th className={`${isDesktop ? "px-6" : "px-4"} text-[9px] bg-[#17152b] text-white/30 uppercase tracking-[0.2em] font-black text-left`}>Asset Identity</th>
                {!isMobile && <th className="px-6 text-[9px] bg-[#17152b] text-white/30 uppercase tracking-[0.2em] font-black text-left">Specifications</th>}
                <th className={`${isDesktop ? "px-6" : "px-4"} text-[9px] bg-[#17152b] text-white/30 uppercase tracking-[0.2em] font-black text-left`}>Market Value</th>
                <th className={`${isDesktop ? "px-6" : "px-4"} text-[9px] bg-[#17152b] text-white/30 uppercase tracking-[0.2em] font-black text-left`}>Status</th>
                <th className="w-20 text-center text-[9px] bg-[#17152b] text-white/30 uppercase tracking-[0.2em] font-black">Action</th>
              </tr>
            </thead>
            <tbody>
              {loading ? (
                <tr>
                  <td colSpan={6} className="py-20 text-center">
                    <div className="flex flex-col items-center gap-3">
                      ...
                      <span className="text-[10px] font-black text-white/20 uppercase tracking-[0.2em]">Syncing...</span>
                    </div>
                  </td>
                </tr>
              ) : filteredUnits.length === 0 ? (
                <tr>
                  <td colSpan={6} className="py-20 text-center text-white/20 font-black uppercase tracking-widest">No matching units</td>
                </tr>
              ) : (
                filteredUnits.map((u, idx) => (
                  <tr key={u.id} className="hover:bg-white/[0.03] transition-colors border-b border-white/5 last:border-0 group">
                    ...
                    <td className={isDesktop ? "px-6 py-5" : "px-4 py-3"}>
                      <div className={`font-black text-[var(--accent)] tracking-tight ${isDesktop ? "text-lg" : "text-sm"}`}>
      ...
      {showModal && (
        <div className="fixed inset-0 z-[1000] flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-lg bg-[#1c1730] rounded-2xl border border-white/10 p-6 sm:p-8 shadow-2xl animate-in zoom-in duration-300">
             <h2 className="text-lg font-black text-white mb-6 uppercase tracking-wider">{editId ? 'Edit Asset' : 'Register Asset'}</h2>
             <form onSubmit={handleSubmit} className="grid grid-cols-2 gap-4">
                <InputField label="Asset ID" name="kode" value={form.kode} onChange={handleChange} required disabled={!!editId} />
                <InputField label="Block Ref" name="blok" value={form.blok} onChange={handleChange} />
                <InputField label="Type" name="tipe" value={form.tipe} onChange={handleChange} />
                <div className="flex flex-col gap-1.5">
                   <label className="text-[9px] font-black text-white/40 uppercase tracking-widest ml-1">Current Status</label>
                   <select name="status" value={form.status} onChange={handleChange} className="h-10 px-3 bg-[#07060d] border border-white/10 rounded-lg text-xs text-white outline-none focus:border-[var(--accent)]">
                     {STATUS_OPTIONS.map(s => <option key={s} value={s}>{s}</option>)}
                   </select>
                </div>
                <InputField label="Land Area (m²)" name="luas_tanah" value={form.luas_tanah} onChange={handleChange} type="number" />
                <InputField label="Building Area (m²)" name="luas_bangunan" value={form.luas_bangunan} onChange={handleChange} type="number" />
                <div className="col-span-2">
                   <InputField label="Valuation (IDR)" name="harga" value={form.harga} onChange={handleChange} type="number" required />
                </div>
                ...
                <div className="col-span-2 flex gap-3 mt-4">
                   <button type="button" onClick={tutupModal} className="flex-1 h-12 rounded-lg bg-white/5 text-white text-xs font-black uppercase tracking-widest">Cancel</button>
                   <button type="submit" className="flex-1 h-12 rounded-lg bg-[var(--accent)] text-black text-xs font-black uppercase tracking-widest">{submitting ? '...' : 'Save Changes'}</button>
                </div>
             </form>
          </div>
        </div>
...
function InputField({ label, ...props }) {
  return (
    <div className="flex flex-col gap-1.5">
      ...
      <input 
        {...props}
        className="h-10 px-3 bg-[#07060d] border border-white/10 rounded-lg text-xs text-white outline-none focus:border-[var(--accent)] transition-all placeholder:text-white/20"
      />
    </div>
  );
}
